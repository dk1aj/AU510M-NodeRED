...
const METER_NAMES = [
    "FWDPWR", "REVPWR", "SWR", "RFPOWER", "MOX",
    "VOLTS", "PATEMPF", "PATEMPC", "MAINFAN",
    "AMPPWR", "AMPID", "AMPSWR", "AMPTEMPF", "AMPTEMPC"
];
...
let latestMeters = Object.fromEntries(METER_NAMES.map(name => [name, "--"]));
let meterSocket;
...
function connectMeters() {
    ...
    meterSocket.on("message", raw => {
        try {
            ...
            if (!packet.meters || typeof packet.meters !== "object" || Array.isArray(packet.meters)) return;
            const meters = { ...packet.meters };
            const aliases = {
                "+13.8A": "VOLTS",
                "AMP PWR": "AMPPWR",
                "AMP ID": "AMPID",
            ...
            for (const [source, target] of Object.entries(aliases)) {
                if (Object.prototype.hasOwnProperty.call(meters, source)) meters[target] = meters[source];
            }
            latestMeters = { ...latestMeters, ...meters };
