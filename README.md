...
and a separate Auto AGC-T Watcher, currently version 4.1.
...
post-AGC `SLC/<active-slice>/AGC` and LEVEL, scans down by 10 until a 2.0 dB LEVEL median
knee bracket is found, then scans only that bracket by 2. Each point uses
400 ms settling and an approximately 800 ms window. Three valid LEVEL samples are
enough to measure; fewer extend the window by 500 ms. P10–P90 spread and MAD
provide quality diagnostics and never stop a scan. The configured final offset
...
needs no watcher version bump. The current release tracks OLD_VERSION 4.0 and NEW_VERSION 4.1 in the central version file.
