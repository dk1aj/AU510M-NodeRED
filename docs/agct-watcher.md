...
Current release: 4.0. `agct-watcher-version.json` holds OLD_VERSION 3.9 and NEW_VERSION 4.0; the core publishes both from the watcher tab environment. The dashboard footer renders `Old: v3.9 | New: v4.0` from that status.
...
Every measured point gets 400 ms settling and an approximately 800 ms window targeting ten valid LEVEL samples, with at least eight valid samples required. The point's LEVEL median, P10, P90, P90–P10 spread and median absolute deviation (MAD) are calculated separately from every other AGC-T position. Spread at most 2.0 dB and MAD at most 0.7 dB is stable. Borderline spread through 3.5 dB, excessive MAD, or insufficient samples triggers a fresh window after 500 ms, then 800 ms. A third failed window aborts with `UNSTABLE LEVEL`; a successful retry clears the warning. Changes in LEVEL median between scan positions do not trigger that error. Requested points remain 100, 90, 80 and so on even when the radio reports a point one unit higher or lower. The core tracks the requested and reported values separately, uses the reported value for measurement records, and accepts a one-point normalization during command readback. A late report within one point of the previous value does not abort the next pending command. Only measured points appear in the trace. The configured final offset remains -1, bounded by the configured minimum and 100. The result displays start 100, measured knee, final AGC-T, measurement count and elapsed scan time. ERROR keeps the original abort reason and elapsed scan time, including after restoration and later status updates. No band result or prior AGC-T is used as a start value.
