...
Each point uses 300 ms settling and a 700 ms measurement window with at least five samples per meter. Requested and actual radio-reported AGC-T are tracked separately; a one-point radio normalization does not change the requested 100, 90, 80 coarse sequence. LEVEL spread and median drift must remain within 2.0 dB; the AGC median must fall at least 2.0 dB from the reference. The UI lists only measured points and shows start, knee, final AGC-T, count and scan time on completion. The original threshold is restored after an abort when RX and the saved slice are available. The abort reason and scan time remain visible in ERROR, including during restoration. No write occurs during TX.
