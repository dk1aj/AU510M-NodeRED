...
2. **Messung starten** performs a single complete run: validate RX and both live meters, measure the baseline without writes, set AGC-T to 100, confirm/read back, settle, descend, locate the knee and apply the final value once. If the actual current threshold is already 100, the unchanged baseline supplies the start reference.
...
A preceding plateau is required; an AGC+ fall of at least 1.5 dB must be confirmed in a second independent window. LEVEL must remain within 1 dB of baseline, and each window's spread must remain within 2 dB. Signal variation aborts instead of triggering continuous regulation. The final value is knee + 1 (bounded to 0–100), written once and confirmed before DONE. These are conservative initial algorithm settings, not hardware-calibrated constants.
